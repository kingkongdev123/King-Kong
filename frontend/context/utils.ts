...
export const getATokenAccountsNeedCreate = async (
  ...
  let instructions: TransactionInstruction[] = [], destinationAccounts: PublicKey[] = [];
...
export const getEscrowBalance = async (connection: Connection) => {
  ...
  const escrowATA = await getAssociatedTokenAccount(escrowVault, BANANA_TOKEN_MINT);
  ...
  const tokenAmount = token ? token : 0;

  return {
    sol: res,
    token: tokenAmount * BANANA_TOKEN_DECIMAL,
  }
}
